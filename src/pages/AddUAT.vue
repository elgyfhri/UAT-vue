<template>
  ...
  <div class="container mt-4">
    <h3 class="mb-3">Input Data UAT</h3>
    ...
    <div class="card shadow-sm h-100">
      ...
      <div class="card-body">
        ...
        <div class="mb-3">
          ...
          <select
            ...
            :class="{'is-invalid': perusahaanError}"
          ...
          <div v-if="perusahaanError" class="invalid-feedback">
            Perusahaan wajib dipilih jika tersedia.
          </div>
        </div>
      </div>
    </div>
      ...
      <button
        ...
        <button type="button" class="btn btn-primary btn-sm mb-3  w-auto" 
      @click="addPageRow">
        Tambah Halaman
    </button>
      </div>
    ...
    <tbody>
      ...
      <template v-for="(page, pageIndex) in filteredPages" :key="'page-' + pageIndex">
        ...
        <tr class="row-pages">
          ...
          <td class="justify-content-between gap-2">
            <button type="button" class="btn btn-info btn-sm" @click="addSection(pageIndex)">
              ➕ Section
            </button>
            <button type="button" class="btn btn-danger btn-sm" @click="removePage(pageIndex)">
              ❌
            </button>
          </td>
        </tr>
...
export default {
  data() {
    return {
      ...
      perusahaanError: false, // Error state untuk validasi perusahaan
...
addSection(pageIndex) {
  ...
  this.updateProgress(); // Update progress setelah menambah section
},
...
updateProgress() {
  ...
  // Validasi Client
  if (this.clients.length > 0 && !this.form.client_id) {
    Swal.fire({
      icon: "error",
      title: "Gagal",
      text: 'Pastikan field "Client" telah dipilih.',
    });
    return;
  }

  // Validasi Perusahaan
  if (this.perusahaans.length > 0 && !this.form.perusahaan_id) {
    Swal.fire({
      icon: "error",
      title: "Gagal",
      text: 'Pastikan field "Perusahaan" telah dipilih.',
    });
    return;
  }
  ...
  if (invalidFields.length) {
    const rowIndex = this.form.pages.findIndex(
      (page) => !page.pages
    ) + 1; // Baris pertama yang kosong
    Swal.fire({
      icon: "warning",
      title: "Peringatan",
      text: `Field "Pages" di baris ${rowIndex} kosong. Harap lengkapi semua field Pages.`,
    });
    console.warn("Terdapat field Pages yang kosong. Harap lengkapi semua field Pages.");
    return;
  }

  // Gabungkan section_number dan section_description menjadi section_on_pages untuk setiap section
  this.form.pages.forEach((page) => {
    page.sections.forEach((section) => {
      section.section_on_pages = `${section.section_number} ${section.section_description}`.trim();
    });
  });
  ...
  axios
    ...
    .then((response) => {
      ...
      Swal.fire({
        icon: "success",
        title: "Sukses",
        text: "UAT berhasil ditambahkan.",
        willClose: () => {
          // Arahkan setelah SweetAlert ditutup
